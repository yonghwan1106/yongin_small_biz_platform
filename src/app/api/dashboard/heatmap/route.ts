...
import { ApiResponse, GyeonggiFootTrafficData } from '@/types';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 날짜 파라미터 (기본값: 2023-12-05, API에 실제 존재하는 데이터)
    // API 데이터: 2023년 12월 데이터 (20231205, 20231217, 20231206, 20231229 등)
    const { searchParams } = new URL(request.url);
    const dateParam = searchParams.get('date');
    const date = dateParam || '20231205'; // 경기도 공공데이터 - 실제 사용 가능한 날짜

    // 유동인구 히트맵 데이터 가져오기
    const heatmapData = await fetchFootTrafficData(location, date);

    return NextResponse.json({
      success: true,
      data: heatmapData,
    } as ApiResponse<GyeonggiFootTrafficData[]>, { status: 200 });
